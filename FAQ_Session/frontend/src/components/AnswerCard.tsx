...
export const AnswerCard: React.FC<AnswerCardProps> = ({ answer, questionAuthorId, isQuestionResolved }) => {
  ...
  return (
    <motion.div
      ...
      className={`p-5 border rounded-xl shadow-sm relative transition-colors ${
        answer.isAccepted
          ? 'border-emerald-400 dark:border-emerald-600 bg-emerald-50/10 dark:bg-emerald-900/10'
          : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'
      }`}
    >
      {/* Pinned Solution ribbon */}
      {answer.isAccepted && (
        <div className="absolute top-[-10px] right-4 bg-emerald-600 dark:bg-emerald-700 text-white px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider flex items-center gap-1 rounded-full shadow-sm border border-emerald-500 dark:border-emerald-600">
          <Check size={10} />
      ...
      <div className="flex items-center justify-between gap-4 mb-4 pb-3 border-b border-slate-100 dark:border-slate-700">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 border border-slate-200 dark:border-slate-600 rounded-full bg-slate-50 dark:bg-slate-700 flex items-center justify-center text-xl shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col items-end gap-1.5">
          ...
          {canDelete && !answer.isAccepted && (
            <button
              ...
              disabled={deleting}
              className="p-1.5 text-rose-500 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 border border-slate-100 dark:border-slate-600 hover:border-rose-200 dark:hover:border-rose-700 rounded transition-colors disabled:opacity-40 cursor-pointer"
              title="Delete Reply"
            >
              <Trash2 size={13} />
            </button>
