    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {question.screenshot && (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[9px] font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800 rounded-md">
              <Camera size={9} />
              Screenshot
            </span>
          )}
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {question.tags.map(tag => (
            <span key={tag} className="px-2 py-0.5 text-[10px] font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-600 rounded-md">
              #{tag}
            </span>
          ))}
        </div>
      ...
      <div className="flex md:flex-col items-end justify-between md:justify-center h-full w-full md:w-32 border-t md:border-t-0 border-slate-100 dark:border-slate-700 pt-3 md:pt-0 pl-0 md:pl-4 flex-shrink-0 gap-2">
        <div className="flex items-center gap-2 self-end">
          <div className="flex flex-col items-end text-right">
            ...
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 truncate max-w-[80px]">{question.author.name}</span>
          </div>
