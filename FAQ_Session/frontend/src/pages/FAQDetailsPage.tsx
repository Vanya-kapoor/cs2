  ...
  if (!question) {
    return (
      <div className="text-center py-12">
        <h2 className="text-2xl font-semibold text-slate-800 dark:text-slate-100 font-sans">Question Not Found</h2>
        <button
  ...
  return (
    ...
    >
      ...
      <div className="p-6 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-2xl shadow-sm space-y-4">
        ...
        {/* Author Panel */}
        <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl w-fit shadow-none">
          <div className="w-8 h-8 rounded-full border border-slate-200 dark:border-slate-600 bg-slate-100 dark:bg-slate-600 flex items-center justify-center text-lg shadow-sm overflow-hidden">
          ...
          <div>
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-xs font-semibold text-slate-800 dark:text-slate-100 leading-none">{question.author.name}</span>
        ...
        {/* Screenshot - visible to all users */}
        {question.screenshot && (
          <div className="rounded-xl overflow-hidden border border-slate-200 dark:border-slate-600 shadow-sm">
            <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-700 border-b border-slate-200 dark:border-slate-600 flex items-center gap-1.5">
              <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">📎 Attached Screenshot</span>
            </div>
            <img
              src={question.screenshot}
              alt="Question screenshot"
              className="w-full max-h-80 object-contain bg-white dark:bg-slate-800 cursor-pointer"
              onClick={() => window.open(question.screenshot!, '_blank')}
              title="Click to open full size"
            />
            <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-700 border-t border-slate-200 dark:border-slate-600">
              <span className="text-[10px] text-slate-400 dark:text-slate-500">Click image to view full size</span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-1.5 pt-2">
          {question.tags.map(tag => (
            <span key={tag} className="px-2 py-0.5 text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-600 rounded-md">
              #{tag}
            </span>
          ))}
        </div>
      ...
      <div className="space-y-4 pt-4">
        <h2 className="font-semibold text-lg text-slate-800 dark:text-slate-100 flex items-center gap-2 font-sans">
          ...
          {question.isAccepted && (
            <span className="px-2.5 py-0.5 text-[10px] bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700 rounded-full font-semibold uppercase tracking-wider">
        ...
        {currentUser ? (
          <form onSubmit={handleSubmitAnswer} className="space-y-3">
            <textarea
              ...
              className="w-full p-3 border border-slate-200 dark:border-slate-600 rounded-lg text-sm font-normal outline-none bg-slate-50/50 dark:bg-slate-700/50 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:bg-white dark:focus:bg-slate-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
              required
            />
            <button
              type="submit"
